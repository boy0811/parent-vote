...
from models import db, Candidate, VotePhase, Vote, User
import pandas as pd
import csv
import chardet
...
def admin_add_candidate():
    ...
    if request.method == 'POST':
        ...
        user = User(username=username, candidate_id=cand.id)
    ...
    return render_template('admin_candidate_form.html', candidate=None, user=None)
...
def admin_edit_candidate(candidate_id):
    ...
    user = User.query.filter_by(candidate_id=candidate_id).first()

    if request.method == 'POST':
        cand.class_name = request.form['class_name']
        cand.parent_name = request.form['parent_name']
        cand.name = request.form['parent_name']
    ...
    return render_template('admin_candidate_form.html', candidate=cand, user=user)
...
def admin_delete_candidates():
    ...
    if ids:
        try:
            ...
            users = User.query.filter(User.candidate_id.in_(ids)).all()
            for u in users:
                db.session.delete(u)
